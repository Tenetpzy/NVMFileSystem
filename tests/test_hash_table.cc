...
protected:
    ...
    void SetUp()
    {
        hash_table_init(&ht, 203, &int_hash_op);
    }
...
//    ASSERT_NE(NULL, (void*)arr);
    ...
    for (int i = 0; i < ht.bucket_array_size; ++i)
    {
...
TEST_F(test_int_hash, multikeyInOneBucketTest)
{
    ...
    // 全部都插入第0个桶
    for (int i = 0; i < len; ++i)
        arr[i].key = i * ht.bucket_array_size;
...
void free_int_count_instance(void *o)
{
    delete (int_count*)o;
}
...
protected:
    ...
    void SetUp()
    {
        hash_table_init(&ht, 205, &int_count_hash_op);
    }

    void TearDown()
    {
        hash_table_destroy(&ht, free_int_count_instance);
        printf("\nfree OK!\n");
    }
};
...
TEST_F(test_int_count_hash, Concurrent_test)
{
    ...
    arr = new int_count*[ht.bucket_array_size];
    for (int i = 0; i < ht.bucket_array_size; ++i)
    {
        arr[i] = new int_count;
        arr[i]->key = i;
